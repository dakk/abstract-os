...
#define FILESYSTEM_TOKEN_MAX_SIZE	256

/** Structure to enumerate a directory */
typedef struct
{
	uint32_t Node;							///< Node id
	char Name[FILESYSTEM_TOKEN_MAX_SIZE];	///< Name of the node
} ReadDir;
...
class Filesystem
{
	public:
		...
		static const unsigned TOKEN_MAX_SIZE	=	FILESYSTEM_TOKEN_MAX_SIZE;
		...
		unsigned getTokenNumber(char *path);
